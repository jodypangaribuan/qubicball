...
>(({ className, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            "rounded-3xl border border-stone/30 bg-white text-deep-forest shadow-soft transition-all duration-500 ease-natural hover:-translate-y-1 hover:shadow-medium",
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <p
        ref={ref}
        className={cn("text-base text-deep-forest/70", className)}
        {...props}
    />
))
